    ...
    public static class AutoMapperConfig
    {
        ...
        public static void RegisterMappings()
        {
            ...
            */
            ...
            Mapper.CreateMap<IndividualProviderModel, IEnumerable<Person>>().ConvertUsing<IndividualToContactPersonsConverter>();
            ...
            Mapper.CreateMap<bool, ICollection<Promotion>>().ConvertUsing<PromoteToPromotionCollectionConverter>();
        ...
        class IndividualToContactPersonsConverter : ITypeConverter<IndividualProviderModel, IEnumerable<Person>>
        {
            public IEnumerable<Person> Convert(ResolutionContext context)
            {
                var source = (IndividualProviderModel)context.SourceValue;
                yield return Mapper.Map<Person>(source);
            }
        }

        class PromoteToPromotionCollectionConverter : ITypeConverter<bool, ICollection<Promotion>>
        {
            public ICollection<Promotion> Convert(ResolutionContext context)
            {
                var source = (bool)context.SourceValue;
                if (!source)
                    return null; // note that I am adding 
                else
                    return new List<Promotion> { Promotion.Create(DateTime.Today, -1, null) };
            }
        }
    }
}